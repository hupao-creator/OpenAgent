...
/** Capture real mounted content once; the execution side never asks DOM for a frame. */
export async function prepareMotionCard(card: HTMLElement, origin: { x: number; y: number }, signal?: AbortSignal, embeddedFonts?: string): Promise<PreparedMotionCard> {
  ...
  const blocks = cardRevealBlocks(card)
  const measurements = blocks.map(block => {
    ...
    const points = measureVisibleCharacters(block, clip, singleLine).map(point => ({ ...point,
      left: point.left / scale, right: point.right / scale, top: point.top / scale,
      bottom: point.bottom / scale, lineHeight: point.lineHeight / scale }))
    if (block.classList.contains('thread-provider-status') && clip.width && clip.height) {
      points.push({ left: 0, right: clip.width / scale, top: 0, bottom: clip.height / scale,
        lineHeight: clip.height / scale, weight: 3.8 })
    }
    return { block, clip, singleLine, points }
  }).filter(({ clip, points }) => clip.width > 0 && clip.height > 0 && points.length)
  const options = { width: width + padding * 2, height: height + padding * 2, pixelRatio: ratio, fontEmbedCSS,
  ...
  try {
    ...
    const [full, shell] = await snapshotSurfaceVariants(card, options, [undefined, xml => {
      xml.querySelectorAll<HTMLElement>(`[${attribute}], [${attribute}] *`).forEach(block => { block.style.visibility = 'hidden' })
    }])
    signal?.throwIfAborted()
    const rect = { x: bounds.left - origin.x, y: bounds.top - origin.y, width: bounds.width, height: bounds.height }
    ...
    for (const [index, { clip, singleLine, points }] of measurements.entries()) {
      // Crop on the same raster grid as the complete card to keep glyph pixels identical.
      const left = Math.max(0, Math.floor((clip.left - bounds.left) / scale * ratio) - 2)
      const top = Math.max(0, Math.floor((clip.top - bounds.top) / scale * ratio) - 2)
      const right = Math.min(width * ratio, Math.ceil((clip.right - bounds.left) / scale * ratio) + 2)
      const bottom = Math.min(height * ratio, Math.ceil((clip.bottom - bounds.top) / scale * ratio) + 2)
      const x = left / ratio, y = top / ratio, w = (right - left) / ratio, h = (bottom - top) / ratio
      const dx = (clip.left - bounds.left) / scale - x, dy = (clip.top - bounds.top) / scale - y
  ...
  finally { measurements.forEach(({ block }) => block.removeAttribute(attribute)) }
}
