  ...
  try {
    ...
    if (data.type === 'resize') {
      ...
      if (destination?.character && destination.context && destination.heldBy === surface) {
        ...
        const identity = (value: typeof description): string => JSON.stringify([value.key, value.activity, value.phase, value.layout, value.role, value.resident?.scope, value.resident?.reply])
