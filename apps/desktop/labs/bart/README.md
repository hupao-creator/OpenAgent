...
- B「顺滑推进」：保留文字重叠部分的屏幕位置，平滑移动到新的圆弧落点；连续增量沿当前运动位置接续。
...
「流入速度」可调 0.5×–2×，默认 1×；「暂停流入」只暂停新文字，保留已锁定的扫读与身体动作，「重放」从第一段开始。文字变化只更新圆弧的可见范围，当前动作完整播放，在下一次自然扫读时读取最新范围，避免每个增量重启动作。离开场景会清理输入计时器，动作和文字参数沿用锁定基准。
