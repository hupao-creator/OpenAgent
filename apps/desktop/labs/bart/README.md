...
- 思考可切换中文、英文、混合与短文本。弧线按实际字宽裁切并渐隐旧内容；56 个 Unicode 码点是流入停止后的视觉尾部长度，输入源独立保留最多 32 Ki 码点的连续窗口和绝对位置，字号与角色尺寸保持一致。
...
- B「顺滑推进」：输入按源窗口的绝对位置追加到独立 FIFO，圆弧以不超过每秒 120 个坐标单位消费队首，不按批次大小加速，也不删除尚未展示的中间文字。SVG 仅持有队首最多 112 个字素，已经离开圆弧的前缀被消费，后续文字才进入展示；输入停止后继续按序读完队列，最终落在最新 56 码点尾部。相同文字的新位置仍是新输入，不靠相似文本猜增量。思考段落、执行标识变化，源窗口存在已知缺口或内容被修订时清空旧队列；眼球和身体继续当前动作。
