...
左侧「运行中兜底」当前采用原版双眼和底部彩虹三点：点径由 4px 放大至 7px，间距 6px，保留接力起伏，六色缓慢循环。暂停会同时冻结起伏和颜色；减少动态效果时保留静态的红、绿、蓝三点。
可用 `/?scene=running&variant=bottom` 直接打开。之前的眼神候选保留用于对照：

| 候选 | variant | 表情 |
| --- | --- | --- |
| 原版 · 彩虹三点 | `bottom` | 正式 Bart 的原始双眼与自然眨眼，放大的彩虹三点独立接力 |
| E1 · 追点目光 | `gaze` | 目光略向下，跟随亮点从左到右移动 |
| E2 · 轻眯跟拍 | `focus` | 保持轻眯，每拍略收紧后舒展 |
| E3 · 三拍眨眼 | `blink-beat` | 前两拍专注，第三个点亮起时轻眨 |

用 `/?scene=running&variant=gaze` 等链接直达候选，默认仍为原版。
候选复用正式 `createCanvasCharacter` 的身体和双眼，不再用三点替代眼睛；眼神和底部三点共用一个暂停、调速、页面可见性的时钟。
原版继续使用正式 `BartLogo` 和 Worker 眨眼；暂停仅冻结三点，三个眼神候选则冻结整个画面。眼神候选仍使用早期的小号单色三点。
「待机对照」恢复原本的双眼并隐藏三点；隐藏页面停止候选绘制，减少动态效果时呈现静态眼神和三点。
重放恢复动画，离开场景释放动画与监听。已淘汰的三点替眼方案不再列在候选中。
这些候选尚未接入正式 Dock 的运行状态，不改变正文输出或等待活动时的状态映射。
