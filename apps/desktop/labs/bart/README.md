...
正式组件直接拥有圆弧几何、文本测量与动画生命周期；眼睛由现有 Worker 绘制。两眼各自围绕中心放大 10%，Worker 与静态 SVG 保持一致。身体使用独立变换层，不覆盖拖动与空间路径变换。连续文字更新不会重启动作；输入或专属角色接管、视图隐藏、减少动态效果和卸载都会释放动画。此前的 Lab DOM 观察适配器与 Vite 源码替换已移除。
