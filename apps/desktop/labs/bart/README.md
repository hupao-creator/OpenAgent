...
左侧「运行中兜底」提供 A「呼吸光环」、B「单点巡航」、C「三点接力」和 D「身体轻摆」，默认选择 C。
可用 `/?scene=running&variant=dots` 直接打开；`variant` 也接受 `halo`、`orbit`、`sway`。
四个候选复用正式 `BartLogo` 的角色与 Worker 眨眼，只在 Lab 添加运行提示。
「待机对照」隐藏提示并停止轻摆；「暂停候选动作」暂停提示，保留角色眨眼；循环时长可调 1.5–4 秒。
隐藏页面会暂停候选动作，减少动态效果时保留静态提示。重放恢复候选动作，离开场景释放角色与监听。
