...
- B「顺滑推进」：连续增量保留已在圆弧内的文字位置，以不超过每秒 120 个圆弧坐标单位推进并缓动落定；不按批次大小加速。展示层只从头部移除已经离开圆弧的字形；快照不再重叠，或积压超过圆弧内文字与最新 56 码点尾部时，整体同步到最新片段，避免删除中间文字后拼出原文不存在的句子。思考段落或执行标识变化时立即清空展示缓冲，眼球和身体继续当前动作。
