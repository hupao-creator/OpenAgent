...
左侧「运行中兜底」默认恢复 C「底部三点（原版）」：复用正式 `BartLogo` 与 Worker 眨眼，保留原来的双眼，三个小点在身体下方接力。可用 `/?scene=running&variant=bottom` 直接打开；暂停仅冻结底部提示。

之前三点临时替代双眼的五个候选保留用于对照：

| 候选 | variant | 表情 |
| --- | --- | --- |
| C1 · 三点接力 | `dots` | 原版：依次轻轻抬起、拉长 |
| C2 · 胶囊波浪 | `capsules` | 圆点依次拉成长眼睛 |
| C3 · 依次眨点 | `blink` | 三点轮流压扁、张开 |
| C4 · 聚散呼吸 | `gather` | 一起靠拢、缩小，再展开 |
| C5 · 双眼化点 | `eyes` | 两枚长眼收成三点，再展开回双眼 |

可用 `/?scene=running&variant=dots` 直接打开 C1，替换 `variant` 可直达其他候选。A、B、D 已移除。
复用正式 `createCanvasCharacter` 的轮廓与眼睛模型，在 Lab 主线程隐藏双眼并绘制三点表情。
「待机对照」恢复原本的双眼与眨眼；「暂停候选动作」冻结画面；循环时长可调 1.5–4 秒。
隐藏页面停止绘制，减少动态效果时呈现静态三点。重放恢复候选动作，离开场景释放动画与监听。
