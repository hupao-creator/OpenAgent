...
it('claude state parser fail-closed mutations reject every malformed single field', () => {
  const base = (): ClaudeThreadState => ({
    version: 1,
    turns: [{
      executionId: 'exec-1',
      createdAt: 1_000,
      ...
      activities: [
        { id: 'act-1', kind: 'command', label: 'ls', status: 'completed', taskId: 'task-1' },
        { id: 'act-2', kind: 'tool', label: 'grep', status: 'completed' }
      ],
      interactions: [{ id: 'int-1', kind: 'permission', title: 'allow', status: 'allowed' }],
      notices: [{ id: 'notice-1', level: 'info', message: 'hi' }],
      timeline: [
        { id: 'tl-1', kind: 'user-message', createdAt: 1_000, promptIndex: 0 },
        { id: 'tl-2', kind: 'assistant', createdAt: 1_001, content: 'answer', status: 'complete' },
        { id: 'tl-3', kind: 'activity', createdAt: 1_002, activity: { id: 'act-1', kind: 'command', label: 'ls', status: 'completed', taskId: 'task-1' } },
        { id: 'tl-4', kind: 'notice', createdAt: 1_002, notice: { id: 'notice-1', level: 'info', message: 'hi' } },
        { id: 'tl-5', kind: 'interaction', createdAt: 1_002, interaction: { id: 'int-1', kind: 'permission', title: 'allow', status: 'allowed' } }
      ],
      usage: { inputTokens: 10, costUsd: 0.5 }
    }],
    nativeNotifications: [{ summary: 'n' }]
  })
  type Mutable = Record<string, unknown>
  const rootOf = (state: ClaudeThreadState): Mutable => state as unknown as Mutable
  const turnOf = (root: Mutable): Mutable => (root.turns as Mutable[])[0]!
  const mutations: ReadonlyArray<{ name: string; apply: (root: Mutable) => void }> = [
    { name: 'root unknown key', apply: root => { root.unknownField = 1 } },
    { name: 'version drift', apply: root => { root.version = 2 } },
    { name: 'primarySessionId and pendingFork together', apply: root => { root.primarySessionId = 's'; root.pendingFork = { sourceSessionId: 'x' } } },
    { name: 'turn unknown key', apply: root => { turnOf(root).unknownField = 1 } },
    { name: 'turn status enum', apply: root => { turnOf(root).status = 'archived' } },
    { name: 'turn text NUL', apply: root => { turnOf(root).text = 'a\0b' } },
    { name: 'finishedAt before createdAt', apply: root => { turnOf(root).finishedAt = 999 } },
    { name: 'usage negative value', apply: root => { (turnOf(root).usage as Mutable).costUsd = -0.5 } },
    { name: 'usage unknown key', apply: root => { (turnOf(root).usage as Mutable).leak = 1 } },
    { name: 'plan step status enum', apply: root => { ((turnOf(root).plan as Mutable[])[0] as Mutable).status = 'archived' } },
    { name: 'activity status enum', apply: root => { (turnOf(root).activities as Mutable[])[0]!.status = 'queued' } },
    { name: 'activity unknown key', apply: root => { (turnOf(root).activities as Mutable[])[0]!.extra = 1 } },
    { name: 'activity id over bound', apply: root => { (turnOf(root).activities as Mutable[])[0]!.id = 'a'.repeat(513) } },
    { name: 'duplicate activity id', apply: root => { (turnOf(root).activities as Mutable[])[1]!.id = 'act-1' } },
    { name: 'interaction kind enum', apply: root => { (turnOf(root).interactions as Mutable[])[0]!.kind = 'email' } },
    { name: 'non-elicitation carries elicitation field', apply: root => { (turnOf(root).interactions as Mutable[])[0]!.url = 'https://x' } },
    { name: 'notice level enum', apply: root => { (turnOf(root).notices as Mutable[])[0]!.level = 'debug' } },
    { name: 'duplicate timeline id', apply: root => { (turnOf(root).timeline as Mutable[])[3]!.id = 'tl-2' } },
    { name: 'timeline createdAt regression', apply: root => { (turnOf(root).timeline as Mutable[])[2]!.createdAt = 500 } },
    { name: 'timeline prompt reference out of range', apply: root => { (turnOf(root).timeline as Mutable[])[0]!.promptIndex = 5 } },
    { name: 'duplicate attachment id', apply: root => { turnOf(root).promptAttachments = [[
      { id: 'file-1', name: 'a.png', mimeType: 'image/png', size: 1, kind: 'image' },
      { id: 'file-1', name: 'b.png', mimeType: 'image/png', size: 1, kind: 'image' }
    ]] } },
    { name: 'native notification unknown key', apply: root => { (root.nativeNotifications as Mutable[])[0]!.extra = 1 } }
  ]
  // Exhaustive by construction: sampling these with fast-check would let a single
  // missing guard hide for a whole run, because one unvisited mutation cannot fail.
  for (const mutation of mutations) {
    const state = base()
    // The untouched base is valid: a throw below can only come from the mutation.
    expect(() => parseClaudeThreadState(structuredClone(state)), mutation.name).not.toThrow()
    const root = rootOf(structuredClone(state))
    mutation.apply(root)
    expect(() => parseClaudeThreadState(root), mutation.name).toThrow()
  }
})
