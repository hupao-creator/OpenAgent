...
it('follows the real turn lifecycle while pacing native activity, then shows the final answer after the display queue drains', () => {
  ...
  act(() => f.source.recordOperation({
    ...
    name: 'thread_status', arguments: {}, createdAt: 1, completedAt: 2
  }))
...
it('shows thread reads as generic tool calls even while their Core audit is running', () => {
  ...
  act(() => f.source.recordOperation({
    ...
    name: 'thread_read', arguments: { threadId: 'another-thread' }, createdAt: 1
  }))
  act(() => f.source.acceptEvent('execution-read', {
    type: 'activity-start', activity: {
      id: 'read-call', kind: 'tool', label: 'Read thread', status: 'running', toolName: 'thread_read'
    }
  }))
  ...
  expect(f.view.container.querySelector('.bart-role-tool-name')).toHaveTextContent('thread_read')
