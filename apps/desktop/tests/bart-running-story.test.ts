...
import { RUNNING_BEAT_MS, RUNNING_CYCLE_BEATS, RUNNING_ORBIT_BEATS, sampleRunningStory } from '../src/renderer/src/bart-motion/running-story'
...
it('keeps three beats at the bottom, then sends each light exactly once around Bart and home', () => {
  ...
  expect(RUNNING_BEAT_MS * RUNNING_CYCLE_BEATS).toBeCloseTo(7676)
  for (let dot = 0; dot < 3; dot++) {
    ...
    for (let step = 0; step <= 100; step++) {
      const point = sampleRunningStory(1.22 + RUNNING_ORBIT_BEATS * step / 100).dots[dot]
    ...
    const landed = sampleRunningStory(1.45 + RUNNING_ORBIT_BEATS).dots[dot]
    expect(landed.x).toBeCloseTo(start.dots[dot].x)
    expect(landed.y).toBeCloseTo(start.dots[dot].y)
  }
  expect(sampleRunningStory(RUNNING_CYCLE_BEATS)).toEqual(start)
})

it('keeps eyes and lights continuous across all stages, with a stable reduced-motion frame', () => {
  for (const at of [1, 1.22, 1.22 + RUNNING_ORBIT_BEATS, 1.45 + RUNNING_ORBIT_BEATS, RUNNING_CYCLE_BEATS]) {
  ...
  expect(sampleRunningStory(1.55).eyes.x).toBeLessThan(0)
  expect(sampleRunningStory(2).eyes.x).toBeGreaterThan(0)
})
