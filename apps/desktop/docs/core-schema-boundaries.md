...
metadata and auto-intervention completion structures. Main continues to own
...
- `apps/desktop/src/main/internal-run-schemas.ts` derives metadata and auto-intervention model JSON Schema from shared representable structures.
...
| Auto-intervention reason | At most 240 Unicode code points after single-line normalization; empty and NUL rejected. |
| Public response message | UTF-16 code units at runtime (including supplementary characters); the model JSON Schema limit is in code points. |
...
Auto-intervention separates model structure, response validation and execution.
A parsed response still requires the currently pending interaction and valid
action at execution time. A `wait` decision requires a null response.
...
- A5: Metadata fallback/tag preservation and auto-intervention validation and state checks remain unchanged.
