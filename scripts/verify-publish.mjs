 ...
 *  pull request contributes only `check-run.json`; the head and the repository the
 *  check run lands on come from the event, which no pull request can forge. */
...
\`verify\` run uploaded. Reads the trusted head from the workflow_run event
(VERIFY_HEAD, VERIFY_EVENT, VERIFY_HEAD_REPOSITORY, GITHUB_REPOSITORY), the payload
from $VERIFY_EVIDENCE/check-run.json, and needs GITHUB_TOKEN to write. Nothing is
published for a run that was not a same-repository pull request, and a missing
payload is left unpublished rather than reported.`)
    return null
  }
  ...
  if (!commitSha.test(head)) return 'the event did not report a head commit'
  ...
  try {
    ...
    // Only the head is bound to the event. The base in the evidence is the one the run
    // resolved at execution time, which legitimately differs from the base the event
    // captured — a base branch can advance while the head stands still — so the gate,
    // not the publisher, is what checks it against the current base.
    if (!proof || proof[1] !== head) return 'the payload evidence does not name this run\'s head'
    publish(payload)
